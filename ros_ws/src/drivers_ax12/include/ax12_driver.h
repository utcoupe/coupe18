...
public:
    ...
    const static uint8_t PING_PASS_NBR = 100; // number of times a motor is pinged to make sure it is connected
    const static uint8_t BAUD_RATE_INDEX = 1; // baudrate = 2000000 / (index + 1)
    const static int PING_SLEEP = 500; // microsec to sleep between pings
