...
from map import Map # TODO remove
...
class CollisionsResolver(object):
    @staticmethod
    def intersects(obs1, obs2):
        if isinstance(obstacle.Shape, RectObstacle):
            return self.point_in_self(obstacle)
        elif isinstance(obstacle.Shape, CircleObstacle):
    ...
    def _point_in_rect(self, point, rect): # Calculs persos, a tester #PS22
        phi = math.atan2(rect.Shape.Height, rect.Shape.Width)
...
class PathChecker(object):
    ...
    def checkCollisions(self, robot, path_shapes, obstacles):
        ...
        print "intersection returned " + str(i._rect_intersects_circle(path_shapes[4], Map.BeltPoints[2]))
        return []
